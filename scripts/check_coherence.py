...
SPEC: list[tuple[str, str, str]] = [
    ...
    # test) holds. The SDK legs are generated by gen_constants.py; the sentinel gate pins every
    # leg's VALUE to 0x1111…1111 so a re-freeze can never move any of them.
    ("sentinel", "client/order.py",               r'PLACEHOLDER_SPENDER\s*=\s*"' + ADDR + r'"'),
